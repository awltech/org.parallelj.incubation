...
public class NewBarChartforProcedure {
	
	public ExtendedItemHandle createMyChart(ElementFactory designFactory,String dataset,int m,String[] procedurelist){
		...
		try{
			...
			cs1.setExpression("dataSetRow[\"Time"+m+"\"]");
			cs2.setExpression("dataSetRow[\"Ave"+m+"\"]");
			cs3.setExpression("dataSetRow[\"Max"+m+"\"]");
			cs4.setExpression("dataSetRow[\"Min"+m+"\"]");
		...
		cwaBar.getTitle().getLabel().getCaption().setValue(procedurelist[m]+" Bar Chart");
		...
		xAxis.getTitle().getCaption().setValue("Type of Time");
		...
		Query xQ = QueryImpl.create( "row[\"Time\"]" );
		seCategory.getDataDefinition( ).add( xQ );
		
		BarSeries bs1 = (BarSeries) BarSeriesImpl.create();
		bs1.setSeriesIdentifier("Average");//Set the label in the right side of chart
		Query yQ1 = QueryImpl.create( "row[\"Average\"]" );
	    ...
	    bs2.setSeriesIdentifier("Maximum");
	    Query yQ2 = QueryImpl.create( "row[\"Maximum\"]" );
	    ...
	    bs3.setSeriesIdentifier("Minimum");
	    Query yQ3 = QueryImpl.create( "row[\"Minimum\"]" );
